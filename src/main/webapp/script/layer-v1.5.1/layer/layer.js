...
Class.pt.space = function(html){
	html = html || '';
	var times = this.index, config = this.config, dialog = config.dialog, frame = [
...
Class.pt.adjust = function(){
	...
	if(config.offset[1].indexOf("px") != -1){
		_left = parseInt(config.offset[1]);
	}else{
		if(config.offset[1] == '50%'){
			_left =  config.offset[1];
		}else{
			_left =  parseInt(config.offset[1])/100*w.width();
		}
	};
	...
	switch(config.type){
		...
		case 4 :
			...
			if(config.tips.guide === 1){
				var offleft = w.width() - left - fWidth - layerE.outerWidth() - 10;
				top = ftop;
				if(offleft > 0){
...
Class.pt.autoArea = function(times){
	//var othis = this;
	var layerE = $('#xubox_layer' + times), config =this.config, page = config.page,
	...
	var btnHeight = 0, outHeight = 0;
	switch(config.type){
		case 0:
			var aBtn = layerBtn.find('a');
			outHeight =  layerText.outerHeight() + 20;
			if(aBtn.length > 0){
				btnHeight = layerBtn.find('a').outerHeight() +  20;
			}else{
				btnHeight = 0;
			}
		break;
		...
		case 3:
			btnHeight = 0;
			outHeight = $(".xubox_loading").outerHeight(); 
...
Class.pt.move = function(){
	var config = this.config, layerMove = this.layerE.find(config.move[0]), layerE = "", ismove = false;
	var moveX = 0, moveY = 0, move = 0, setY = 0;
	...
	$(document).mousemove(function(M){			
		...
		if(ismove){
			...
			if(parseInt(layerE.css('margin-left')) == 0){
				lefts = parseInt(move.css('left'));
			}else{
				lefts = parseInt(move.css('left')) + (-parseInt(layerE.css('margin-left')));
			}
...
Class.pt.IE6 = function(){
	//var othis = this;
	var layerE = this.layerE, select = $('select');
